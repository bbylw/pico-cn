...
title: CSS 变量
chapter: 定制
lead: 通过 130 多个 CSS 变量定制 Pico 的设计系统，打造独一无二的外观与质感。
toc:
  - { id: example, text: 示例, depth: 2 }
  - { id: css-variables-for-color-schemes, text: 配色方案的 CSS 变量, depth: 2 }
  - { id: all-css-variables, text: 全部 CSS 变量, depth: 2 }
---

import Ex from "../../components/Ex.astro";

Pico 包含许多自定义属性（变量），让你轻松访问 `font-family`、`font-size`、`border-radius`、`margin`、`padding` 等常用值。

所有 CSS 变量都带有 `pico-` 前缀，以避免与其他 CSS 框架或你自己的变量发生冲突。你可以通过 [SASS](/docs/sass/) 重新编译 CSS 文件来移除或自定义这个前缀。

你可以在 `:root` 选择器内定义 CSS 变量以全局应用更改，也可以在特定选择器上覆盖 CSS 变量以局部应用更改。

<h2 id="example">示例</h2>

<Ex
  html={`<article aria-label="Button colors example" class="component" id="css-var-example"><style>
 #css-var-example h1,
 #css-var-example p,
 ...
 </style><h1>Music fest mania</h1><p>Get ready to dance and sing your heart out at our Music Fest Mania. Join the crowd, jam to your favorite band, and discover new artists.</p><button>Let’s rock out!</button></article>`}
  code={`<style>
  :root {
    --pico-border-radius: 2rem;
    --pico-typography-spacing-vertical: 1.5rem;
...
<h2 id="css-variables-for-color-schemes">配色方案的 CSS 变量</h2>

若只想为浅色模式（默认模式）添加或编辑 CSS 变量，请在以下选择器内部定义：
...
若要为深色模式添加或编辑 CSS 变量，你需要定义两次。

第一处是在 `@media` 查询中，通过 `prefers-color-scheme: dark` 检查用户是否在设备设置中启用了深色模式。在这种情况下，如果没有显式设置 `data-theme` 属性，深色模式的样式就会应用到 `:root` 元素。

第二处是当你用 `data-theme="dark"` 强制启用深色模式时。这让你可以不受用户设备设置的影响，手动在浅色与深色主题之间切换。
...
<h2 id="all-css-variables">全部 CSS 变量</h2>
...
以下是 Pico 中使用的全部 CSS 变量列表：

<details>
  <summary>默认样式 CSS 变量</summary>
