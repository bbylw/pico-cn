...
- **首屏（首页）**：全幅出血 hero（负 margin 破 container，内层 container 对齐）+ 超大展示字（code-chip 签名）+ 组件橱窗（搜索/按钮/开关/滑块/手风琴真实渲染）；收尾行动面板用主色反白收束
- **信息密度（首页）**：bento 演示收进 tinted 演示区；起步代码加行号；文档地图为带页数的索引卡片（组标题直达该组首篇）；页脚为品牌 + 文档 + 资源三栏
